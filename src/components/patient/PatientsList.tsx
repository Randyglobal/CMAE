...
interface Patient {
  ...
  lastVisit: string;
  phone: string;
  status: "Stable" | "Critical" | "Discharged" | "Observation";
  room?: string;
}

interface SortState {
  sortBy: "mrn" | "name" | "dob" | "lastVisit" | "phone";
  sortDirection: "asc" | "desc";
}

const initialPatients: Patient[] = [
  { id: 1, mrn: "MRN-0001", name: "Lindsey Curtis", dob: "1986-04-12", lastVisit: "2026-03-25", phone: "(555) 123-4567", status: "Stable", room: "101A" },
  { id: 2, mrn: "MRN-0002", name: "John Doe", dob: "1978-09-03", lastVisit: "2026-03-20", phone: "(555) 987-6543", status: "Observation", room: "202B" },
  { id: 3, mrn: "MRN-0003", name: "Jane Smith", dob: "1992-12-22", lastVisit: "2026-02-18", phone: "(555) 555-1212", status: "Stable", room: "103" },
  { id: 4, mrn: "MRN-0004", name: "Michael Brown", dob: "1965-06-30", lastVisit: "2026-03-28", phone: "(555) 222-3333", status: "Critical", room: "ICU-1" },
  { id: 5, mrn: "MRN-0005", name: "Emily Davis", dob: "1989-01-05", lastVisit: "2026-03-10", phone: "(555) 444-5555", status: "Stable", room: "104" },
  { id: 6, mrn: "MRN-0006", name: "Chris Wilson", dob: "2000-07-17", lastVisit: "2026-01-15", phone: "(555) 666-7777", status: "Discharged" },
  { id: 7, mrn: "MRN-0007", name: "Jessica Lee", dob: "1975-11-02", lastVisit: "2026-03-22", phone: "(555) 888-9999", status: "Observation", room: "205" },
  { id: 8, mrn: "MRN-0008", name: "David Kim", dob: "1990-05-14", lastVisit: "2026-02-28", phone: "(555) 101-2020", status: "Stable", room: "106" },
  { id: 9, mrn: "MRN-0009", name: "Sarah Clark", dob: "1983-08-08", lastVisit: "2026-03-01", phone: "(555) 303-4040", status: "Stable", room: "107" },
  { id: 10, mrn: "MRN-0010", name: "Matthew Lewis", dob: "1959-10-21", lastVisit: "2026-03-30", phone: "(555) 505-6060", status: "Critical", room: "ICU-2" },
];
...
const PatientsListTable: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "All" | "Stable" | "Critical" | "Discharged" | "Observation"
  >("All");
  ...
  const filteredPatients: Patient[] = useMemo(() => {
    return filterStatus === "All"
      ? patients
      : patients.filter((p) => p.status === (filterStatus as Patient['status']));
  }, [patients, filterStatus]);

  const searchedPatients: Patient[] = React.useMemo(() => {
    return filteredPatients.filter((p) =>
      (`${p.mrn} ${p.name} ${p.phone}`).toLowerCase().includes(search.toLowerCase())
    );
  }, [filteredPatients, search]);

  const sortedPatients: Patient[] = React.useMemo(() => {
  ...
  }, [searchedPatients, sort]);

  const paginatedPatients: Patient[] = sortedPatients.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  const totalPages: number = Math.ceil(sortedPatients.length / itemsPerPage) || 1;
  const startEntry: number = sortedPatients.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endEntry: number = Math.min(currentPage * itemsPerPage, sortedPatients.length);
  ...
  const toggleSelectAll = (): void => {
    if (
      ...
      paginatedPatients.every((i) => selected.includes(i.id))
    ) {
      setSelected([]);
    } else {
      setSelected(paginatedPatients.map((i) => i.id));
    }
  };
  ...
  const sortBy = (field: SortState['sortBy']): void => {
    setSort((prev) => ({
      sortBy: field,
      sortDirection:
              ...
              >
                <div className="flex w-full cursor-pointer items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <p className="text-theme-xs font-medium text-gray-700 dark:text-gray-400">Patient</p>
              ...
              >
                <td className="p-4 whitespace-nowrap">
                  <div className="group flex items-center gap-3">
                    <label className="flex cursor-pointer items-center text-sm font-medium text-gray-700 select-none dark:text-gray-400">
                      <span className="relative">
                        ...
                        >
                          <span
                            className={
                              selected.includes(patient.id) ? "" : "opacity-0"
                            }
                          >
                            <svg
                              width="12"
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-sm font-medium text-gray-600">{patient.name.split(' ').map(n=>n[0]).slice(0,2).join('')}</div>
                      <div className="flex flex-col">
                        <span className="text-theme-xs font-medium text-gray-700 group-hover:underline dark:text-gray-400">{patient.name}</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400">{patient.mrn}</span>
                      </div>
                    </div>
                  </div>
                </td>
                <td className="p-4 whitespace-nowrap">
                  <span className="text-sm text-gray-700 dark:text-gray-400">{patient.dob}</span>
                </td>
                <td className="p-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">{patient.lastVisit}</p>
                </td>
                <td className="p-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">{patient.phone}</p>
                </td>
                <td className="p-4 whitespace-nowrap">
                  <span
      ...
      <div className="flex items-center flex-col sm:flex-row justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
        <div className="pb-3 sm:pb-0">
          <span className="block text-sm font-medium text-gray-500 dark:text-gray-400">
            ...
            of {" "}
            <span className="text-gray-800 dark:text-white/90">{sortedPatients.length}</span>
          </span>
        </div>
